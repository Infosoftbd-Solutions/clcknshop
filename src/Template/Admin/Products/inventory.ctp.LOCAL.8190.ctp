    ...
    <table cellpadding="0" cellspacing="0" class="table table-hover table-outline table-vcenter text-nowrap card-table">
        ...
        <tbody>
            <?php foreach ($products as $product):
               $has_variant = (sizeof($product->product_variants) > 0)?true:false;
              ?>
            <tr>
                ...
                <td style="width:120px"><img src="https://preview.tabler.io/demo/products/apple-macbook-pro.jpg" alt="" class="h-8"></td>
                 ...
                 <td><?php  $sku = $this->Html->link(empty($product->sku)?"Set sku now":$product->sku,"javascript:;;",['class'=>'skulink']);  echo (!$has_variant)?$sku:"";  ?></td>
            ...
            <?php  foreach($product->product_variants as $variant):?>
              ...
              <tr class="row_<?=$product->id?>">
                  ...
                  <td ></td>
                   ...
                   <td><?= $this->Html->link(empty($variant->sku)?"Set sku now":$variant->sku,"javascript:;;",['class'=>'skulink','data-pk'=>$variant->id. "_" . $product->id ]) ?></td>
                   <td><?=$this->Html->link(($variant->sell_w_stock==true)?"Don't stop selling":"Stop selling","javascript:;;",['class'=>'selllink','data-value'=>($variant->sell_w_stock)?1:0,'data-pk'=>$variant->id. "_" . $product->id ])  ?></td>
                   <td class="colavailable"><?= $variant->q_available ?></td>
                   <td class="text-right">
                    <?=$this->Form->create(null,['url'=>['action'=>'updateinventory',$product->id,$variant->id]])?>
                    <div class="row">
                    <div class="col-lg-3">
                        <?=$this->TablerForm->control('set',['label'=>false,'options'=>['Add','Set']])?>
                        <?=$this->Form->hidden('id',['value'=>$product->id])?>
                        <?=$this->Form->hidden('variant_id',['value'=>$variant->id])?>
