    ...
    require(['jquery', 'flatpickr'], function ($, selectize) {
        $(document).ready(function () {
            ...
            $(".calendar-range").flatpickr({
                mode: "range",
                dateFormat: "d-m-Y",
                defaultDate: [last_date(30), new Date()]
            })
            ...
            function filter() {
                $("#content").html(placeholder(2))
                let range = $("#filter_range").val();
                let routeUrl = "<?= $this->Url->build(['controller' => 'Reports', 'action' => 'finance']) ?>/"+encodeURIComponent(range);
                $.ajax({
                    url: routeUrl,
                    type: 'GET',
            ...
            function placeholder(loop_limit = 1){
                let ph = ''
                for (let i = 1; i <= loop_limit; i++){
                    ph +='<div class="col-md-6 px-lg-5 mb-5">'
                    ph += '<div class="card">'
                    ph +='<div class="ph-item" style="border: none">'
                    ph +='<div class="ph-col-12">'
                    ph +='<div class="ph-row">'
                    ph +='<div class="ph-col-10 big" style="height: 30px"></div>'
                    ph +='</div>'
                    ph +=''
                    ph +='<div class="ph-row">'
                    ph +='<div class="ph-col-6 big"></div>'
                    ph +='<div class="ph-col-2 big empty"></div>'
                    ph +='<div class="ph-col-4 big"></div>'
                    ph +=''
                    ph +='<div class="ph-col-6 big"></div>'
                    ph +='<div class="ph-col-2 big empty"></div>'
                    ph +='<div class="ph-col-4 big"></div>'
