    ...
    require(['jquery', 'flatpickr'], function ($, selectize) {
        $(document).ready(function () {
            ...
            $("#export").click((e) => {
                ...
                let routeUrl = "<?= $this->Url->build(['controller' => 'Reports', 'action' => 'productOrdersReports']) ?>/"+encodeURIComponent(range) + '/1';
                $.ajax({
                    url: routeUrl,
                    type: 'GET',
            ...
            function filter() {
                ...
                let range = $("#filter_range").val();
                let routeUrl = "<?= $this->Url->build(['controller' => 'Reports', 'action' => 'productOrdersReports']) ?>/"+encodeURIComponent(range);
                $.ajax({
                    url: routeUrl,
                    type: 'GET',
                    // dataType: 'json', // added data dtype
